...
public class Tableau {
   ...
   */
    private SpaceShip getRightMostEnemy() {
    	for(int y = 0; y < InvaderShip.SHIPS_Y; y++)
    		for(int x = InvaderShip.SHIPS_X-1; x >= 0; x--)
    			if(enemyFleet[y][x] != null)
   ...
   */
	private SpaceShip getLowestEnemy() {
		for(int y = InvaderShip.SHIPS_Y-1; y >= 0; y--)
    		for(int x = InvaderShip.SHIPS_X-1; x >= 0; x--)
    			if(enemyFleet[y][x] != null)
    				return enemyFleet[y][x];
    	return null;
   ...
   */
    public void moveEnemyFleet() {
    	...
    	{	
    		...
    		if(fleetMovePositiveX)
    			translateEnemyFleet(FLEET_MOVE_X, deltaY);
    		else
    			translateEnemyFleet(-1*FLEET_MOVE_X, deltaY);
    	}
	}
